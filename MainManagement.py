...
from Reports_ui import ReportManagement     #### Ratio Multiplier for Different screen to adjust the program
...
class MainManagement(QMainWindow):
    ...
    def initUI(self):
        ...
        self.stock_management=StockManagement(self.main_stacked_widget,self.main_menu_screen)
        self.customer_management=CustomerManagement(self.main_stacked_widget,self.main_menu_screen)
        self.rent_management=RentManagement(self.main_stacked_widget,self.main_menu_screen)
        self.report_management=ReportManagement(self.main_stacked_widget,self.main_menu_screen)
        self.sale_management=SaleManagement(self.main_stacked_widget,self.main_menu_screen)
    ...
    def create_main_screens(self):
        modules = [
            ...
            ("Customer Management", self.customer_management.setup_customer_management_screen),
        ...
        management_layout,self.main_menu_screen=setup_options_management_screen(self.main_stacked_widget,"Stock Management & Rental System",modules,provide_theme_buttons=True,main_object=self)
         # Add progress label
        self.progress_label.setAlignment(Qt.AlignCenter)
        self.progress_label.setStyleSheet(f"font-size: {int(20*SCRN_RATIO)}px; color: blue;")

        # self.progress_label.setAlignment(Qt.AlignBottom)
        self.progress_label.setFixedHeight(int(20*SCRN_RATIO))  # Adjust height as needed
    ...
    def upload_to_google_drive(self):
        try:
            ...
            file_list = drive.ListFile({'q': f"title='{db_file_name}'"}).GetList()
            old_file_name = None  # Initialize variable to handle the deletion step
            
            if file_list:
                self.progress_label.setText("Renaming the existing file on Google Drive...")
                QApplication.processEvents()
                ...
                self.progress_label.setText(f"Renamed existing file to '{old_file_name}'.")
                QApplication.processEvents()
            else:
                self.progress_label.setText("No existing file found. Proceeding with the upload...")
                QApplication.processEvents()
            ...
            self.progress_label.setText(f"File '{db_file_name}' uploaded successfully.")
            QApplication.processEvents()

            # Optionally, delete the renamed file if it exists
            if old_file_name:
                self.progress_label.setText(f"Deleting the old database file '{old_file_name}'...")
                QApplication.processEvents()
                old_file.Delete()
                self.progress_label.setText(f"Deleted the previous version '{old_file_name}'.")
                QApplication.processEvents()
                self.progress_label.setText("Database upload completed successfully.")
                QApplication.processEvents()
                sleep(2)
